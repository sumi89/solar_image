...
def get_urls_of_imgs_w_wavelength_resolution(date1, date2, wavelength, resolution, url):
    ...
    #    page = requests.get(url_dates[i])    
        ...
        if not img_files_wr:
            for i in range(0,24):
                required_urls.append(required_urls[-1]) 
                #print('empty',required_urls[-1])
...
######### creating images with false channels ###########
image_height = 472
image_width = 472
channels = 3
...
######### saving images with false channels ###########
for i in range(0,len(required_urls1)):
    print('i = ', i)
